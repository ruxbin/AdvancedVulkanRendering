...
// Vulkan uses set 1 here, not set 0: GpuScene.cpp builds _hizPipelineLayout as
// {set 0 = copy layout, set 1 = downsample layout} and binds the downsample set
// with firstSet=1. DX12 has a single root signature shared by both kernels, so
// the registers stay at t0/u1 to match _hizRootSig's descriptor table.
VK_BINDING(0,1) Texture2D<float> prevMip REGISTER_SRV(0,0);
VK_BINDING(1,1) RWTexture2D<float> currentMip REGISTER_UAV(1,0);
