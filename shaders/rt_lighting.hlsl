...
float3 skyColor(float3 dir) {
    ...
    return lerp(horizon, zenith, t * t) * 2.0f;
}
...
HitInputs gatherHit(BuiltInTriangleIntersectionAttributes attribs) {
    ...
    h.geoN = normalize(bary.x * vbNormals[idx.x]  + bary.y * vbNormals[idx.y]  + bary.z * vbNormals[idx.z]);
    h.geoT = normalize(bary.x * vbTangents[idx.x] + bary.y * vbTangents[idx.y] + bary.z * vbTangents[idx.z]);
    h.uv   =           bary.x * vbUVs[idx.x]      + bary.y * vbUVs[idx.y]      + bary.z * vbUVs[idx.z];
...
void RayGen() {
    ...
    // Per-pixel RNG: mix pixel coords, frame seed, and accum count.
    uint rng = (px.x * 1920u + px.y) ^ (pc.frameSeed * 2654435761u)
